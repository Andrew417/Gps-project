...
void GPIO_init()
{
	...
	GPIO_PORTE_AMSEL_R &= ~0x32;	  // Disable analog on selected pins
	GPIO_PORTE_PCTL_R &= ~0x32;	  // Clear PCTL bits for selected pins
	GPIO_PORTE_AFSEL_R &= ~0x32;	  // Choose alternate functions on selected pins
